## 1.4 - 2026-09-14
* Карта ПВЗ: каждая точка загружается один раз, все её тарифы показываются в одном балуне. Меньше памяти и трафика на больших городах.
* Формат ответа карты изменён: `ApishipMap.open` принимает весь ответ сервера (важно для доработанных шаблонов).
* OpenCart 2.x/3.x: cron-эндпоинты экспорта и импорта статусов принимают только POST, ключ передаётся заголовком `X-Apiship-Key`. Cron-команды нужно перенастроить, пример в настройках модуля.
* OpenCart 2.x/3.x: действия с заказом из админки выполняются под сессией администратора, cron-ключ в html админки не попадает.
* OpenCart 2.x/3.x: данные от API экранируются в чекауте, на карте и в админке.
* OpenCart 2.x/3.x: справочники, точки и расчёты кешируются в кеше OpenCart, а не в сессии покупателя. Меньше запросов к API, быстрее карта.
* OpenCart 2.x/3.x: фильтры по типу точки и службе доставки на карте применяются вместе.
* Исправлена ошибка: при редактировании заказа в админке список ПВЗ приходил без названий.
* Исправлена ошибка распределения веса места по позициям при расхождении весов.

## 1.3 - 2026-09-04
* Добавлен пакет для OpenCart 4.1.x (папка `4.x`, архив `apiship.ocmod.zip`): установка через Установщик, карта ПВЗ в штатном чекауте, вкладка ApiShip в заказе, ярлыки и акты, cron-экспорт и импорт статусов.
* OpenCart 4: cron-эндпоинты принимают только POST, ключ передаётся заголовком `X-Apiship-Key`. Cron-команды перегенерированы в настройках модуля.
* OpenCart 4: действия с заказом из админки требуют права на маршрут `extension/apiship/sale/order` (см. README).
* OpenCart 4: справочники API кешируются в кеше OpenCart, а не в сессии. Исправлена «Ошибка расчета стоимости» при выборе ПВЗ у магазинов с несколькими службами доставки.
* OpenCart 4: карта ПВЗ на больших городах не падает по памяти, меньше трафика, быстрее загрузка.
* Исправлена ошибка (OpenCart 4): при редактировании заказа в админке не менялся ПВЗ.
* Исправлена ошибка (OpenCart 4): после обновления модуля поиск ПВЗ в админке не работал из-за старого скрипта в кеше браузера.
* Стенд OpenCart 4.1 в Docker (`dev/oc4`).
