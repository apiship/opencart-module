 ...
 * - действия с заказом из админки (POST + user_token администратора + право modify на sale/order):
...
class ControllerExtensionShippingApiship extends Controller {
	...
	private function admin_guard() {
		...
		$token = (isset($this->request->post['user_token']) && is_scalar($this->request->post['user_token'])) ? (string)$this->request->post['user_token'] : '';
